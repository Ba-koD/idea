...
- `Project State`는 platform backend의 암호화된 state DB에 저장된다.
...
- provisioning은 기존 runtime `terraform.tfstate` 또는 저장된 target id가 있으면 재사용을 먼저 시도한다.
...
- 실제 Ncloud provisioning 구현에는 최소 `region_code`, `zone_code`, `cluster_name`, `vpc_no`, `subnet_no`, `lb_subnet_no`, `node_pool_name`, `node_count`, `node_product_code`(`serverSpecCode`, 예: `s2-g3a`), `block_storage_size_gb`, `access_key_secret_ref`, `secret_key_secret_ref`가 필요하다.
